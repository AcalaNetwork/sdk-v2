...
function transformData(ledger: AcalaPrimitivesBondingLedgerBondingLedger): ACAStakingLedger {
  const unlockingList = ledger.unlocking.map((item) => ({
    amount: item.value.toBigInt(),
    at: item.unlockAt.toBigInt(),
  }));
  const unlockingAmount = unlockingList.reduce((acc, item) => acc + item.amount, 0n);
...
export const getUserACAStakingLedger = async (api: ApiPromise, address: UnifyAddress): Promise<ACAStakingLedger> => {
  const account = await getAccount(api, address);
  const ledger = (await api.query.earning.ledger(account.address)).unwrapOrDefault();
