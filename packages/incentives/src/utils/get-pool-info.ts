...
export async function getPoolDeductionConfigs(
  ...
  const specifiedDeductionToken =
    await api.query.incentives.claimRewardDeductionCurrency(poolId);
  const rewardTokens =
    await api.query.incentives.incentiveRewardAmounts.entries(poolId);
...
export async function getPoolInfo(
  ...
  const { totalShares, rewards } = await getPoolRewardsAndStakedInfo(
    api,
    poolId,
  );
