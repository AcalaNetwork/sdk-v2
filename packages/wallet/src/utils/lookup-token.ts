 ...
 */
export async function lookupToken(api: ApiPromise, tokenSymbolOrId: string): Promise<Token> {
  let token: Token | undefined;
  // when the input token starts with 0x, should check if it's a valid token id, otherwise, it's a symbol
  const isValidTokenId = tokenSymbolOrId.startsWith("0x") ? isTokenId(api, tokenSymbolOrId) : false;
