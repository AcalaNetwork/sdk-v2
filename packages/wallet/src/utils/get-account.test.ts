...
import { computeDefaultEvmAddress, computeDefaultSubstrateAddress, getEvmAddressFromDefaultSubstrateAddress, isDefaultSubstrateAddress } from "./get-account.js";
...
describe("getAccount", () => {
  ...
  it("should compute the default evm address", () => {
    const evmAddress = computeDefaultEvmAddress("5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY");
    expect(evmAddress).toBe("0x82A258cb20E2ADB4788153cd5eb5839615EcE9a0");
  });

  it("should compute the default substrate address", () => {
    const substrateAddress1 = computeDefaultSubstrateAddress("0x818A6414d1E077b8d344316d5A9339431573c3dC");
    // to ensure that formatted address will not affect the result
    const substrateAddress2 = computeDefaultSubstrateAddress(getAddress("0x818A6414d1E077b8d344316d5A9339431573c3dC") as EvmAddress);
    expect(substrateAddress1).toBe("5EMjsczn8bxBoc6qxQR3NMZPRa9Nu4z4LUi5dozEB74qXyrn");
    expect(substrateAddress2).toBe("5EMjsczn8bxBoc6qxQR3NMZPRa9Nu4z4LUi5dozEB74qXyrn");
  });

  it("should check if the address is a default substrate address", () => {
    expect(isDefaultSubstrateAddress("5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY")).toBe(false);
    expect(isDefaultSubstrateAddress("5EMjsd1AzSgP46hVW4AYjayfoJydAknw2ATxGwcGazDCSkfZ")).toBe(true);
  });

  it("should get the evm address from the default substrate address", () => {
    const evmAddress = getEvmAddressFromDefaultSubstrateAddress("5EMjsczn8bxBoc6qxQR3NMZPRa9Nu4z4LUi5dozEB74qXyrn");
    expect(evmAddress).toBe(getAddress("0x818A6414d1E077b8d344316d5A9339431573c3dC"));
  });
  ...
  it("should get the account info from the substrate address", async () => {
    ...
    const substrateAddress = formatSubstrateAddress(api, "25E8NfHMza8XaA7AQvwn1SMFMbk4nZmqmcXF2LbSCurWV2Cr");
  ...
  it("should get the account info from the evm address with unbound substrate address", async () => {
    ...
    const substrateAddress = formatSubstrateAddress(api, computeDefaultSubstrateAddress(address));
  ...
  it("should error when the address is not a valid substrate address", async () => {
    await expect(getAccount(api, "not a valid substrate address")).rejects.toThrow();
  });
});
